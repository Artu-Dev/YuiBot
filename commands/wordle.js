...
import { getOrCreateUser, addChars, reduceChars, getServerConfig } from "../database.js";
...
async function startGame(client, channel, initialMsg, answer, players, guildId) {
  ...
  collector.on("collect", async (msg) => {
    ...
    if (won) { await willDelete; return finish(true, msg); }
    if (attempts.length >= MAX_ATT) { await willDelete; return finish(false); }

    currentMsg = await replaceMessage(currentMsg,
      buildGamePayload(attempts, usedLetters,
        `Tentativas: ${attempts.length}/${MAX_ATT}  •  Última: ${players.get(msg.author.id)} → ${guess}`,
        { title: "Termoo da YUI!", description: `**Jogadores:**\n${playerList()}` }
      )
    );
  });
...
export async function execute(client, data) {
  ...
  const dailyWord = getOrCreateDailyWord(guildId);
  ...
  const joinRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("termoo_join").setLabel("Entrar no jogo!").setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId("termoo_start").setLabel("COMEÇAR LOGO!").setStyle(ButtonStyle.Success)
  );
